import { ActorGroup, ActorGroupSound, knownActorGroups } from './config'
...
const flipTokens = async (htm: JQuery, ownedTokens: FoundryVTT.TokenPF[]) => {
...
const startSound = async (sound: ActorGroupSound) => {
...
const getSelectElementValue = (htm: JQuery, selector: string): string => {
...
const editInnerHtml = (htm: JQuery, selector: string, value: string) => {
  const element = htm.find(selector)?.[0]
...
const createForm = (actorGroupNames: Set<string>) => `
    <form class="flexcol">
...
const createImageOptions = (htm: JQuery) => {
